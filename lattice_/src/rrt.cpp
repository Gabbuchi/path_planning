...
std::vector<Node*> RRT::Planning() {
  ...
  while (1) {
    ...
    // td::random_device rd;  // 用于获取随机数种子
    // std::mt19937 gen(rd()); // 使用Mersenne Twister算法生成随机数
    // std::uniform_int_distribution<> dis(1, 10); // 定义一个均匀分布的随机数生成器，范围是1到10
    // int random_number = dis(gen); // 生成随机数
    // 此处随机数生成未完善
    if (goal_dis_(goal_gen_) > goal_sample_rate_) { //如果生成的随机数大于目标采样概率，则生成随机点
      double randX = area_dis_(goal_gen_) * 10; //生成随机点X,Y
      double randY = area_dis_(goal_gen_) * 10;
